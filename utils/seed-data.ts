...
export async function seedDatabase() {
  try {
    ...
    for (const apt of mockApartments) {
      ...
      const embedding = await VoyageEmbedding.getEmbedding(
        embeddingText,
...
function getCategoryForFeature(feature: string): string {
  const categories = {
    amenity: [
      'internet', 'smart_home', 'modern_appliances', 'elevator', 'security',
      'pool', 'gym', 'fireplace', 'parking'
    ],
